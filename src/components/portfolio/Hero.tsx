...
export function Hero() {
  return (
    <section
      id="home"
      className="relative min-h-screen flex items-center pt-24 sm:pt-28 pb-20 overflow-x-hidden overflow-y-hidden"
    >
      ...
      <div className="container-portfolio relative z-10 grid lg:grid-cols-12 gap-10 lg:gap-12 items-center">
        <div className="lg:col-span-7 space-y-5 sm:space-y-7">
          <Reveal delay={60}>
            <h1
              className="font-heading font-semibold tracking-tight text-[var(--heading)] break-words"
              style={{ lineHeight: 1.05, textWrap: "balance", overflowWrap: "break-word" } as React.CSSProperties}
            >
              <span className="block text-[clamp(1.75rem,7vw,5rem)]">
                Hello, I&apos;m{" "}
                <span className="text-[var(--accent)]">Error404</span>
              </span>
              <span className="block mt-2 text-[clamp(1rem,4vw,2.6rem)] font-normal text-[var(--subtext)]">
          ...
          <Reveal delay={140}>
            <p className="max-w-[560px] text-[14px] sm:text-[15px] leading-relaxed text-[var(--subtext)]">
          ...
          <Reveal delay={300}>
            <div className="grid grid-cols-3 gap-3 sm:gap-6 sm:flex sm:items-center sm:gap-8 pt-4 border-t border-[var(--border)]">
              {STATS.map((s) => (
                <div key={s.label} className="flex flex-col min-w-0">
                  <span className="font-heading text-[clamp(1.15rem,5vw,1.5rem)] font-semibold text-[var(--heading)] leading-none">
                    {s.value}
                  </span>
                  <span className="font-mono text-[9px] sm:text-[11px] text-[var(--icon-muted)] mt-1.5 uppercase tracking-wider leading-tight">
                    {s.label}
                  </span>
                </div>
              ))}
